...
class UserFeedbackLogic {
    formatMultipleCorrectionsExplanation(response) {
    ...
    generateCorrectionTypeLabel(correctionType) {
        const labels = {
    ...
    validateFeedbackQuality(response) {
    ...
    formatProgressMessage(step, progress) {
...
describe('User Feedback', () => {
    ...
    describe('Property-Based Tests', () => {
        ...
        test('should provide individual explanations for multiple corrections', () => {
            ...
            fc.assert(fc.property(multipleChangesResponseArb, (response) => {
        ...
        test('should generate clear labels for all correction types', () => {
            ...
            fc.assert(fc.property(correctionTypeArb, (correctionType) => {
                ...
                if (correctionType !== CorrectionType_1.CorrectionType.CUSTOM) {
        ...
        test('should validate feedback quality consistently', () => {
            ...
            const responseArb = fc.record({
                ...
                explanation: fc.oneof(fc.string({ minLength: 1 }), fc.constant(''), fc.string().filter(s => s.trim() === '')),
                changes: fc.oneof(fc.array(fc.record({
                    original: fc.string(),
                    corrected: fc.string(),
                    reason: fc.string(),
                    position: fc.record({
                ...
                confidence: fc.oneof(fc.float({ min: 0, max: 1 }), fc.float({ min: Math.fround(-1), max: Math.fround(-0.1) }), fc.float({ min: Math.fround(1.1), max: Math.fround(2) }))
            });
        ...
        test('should format progress messages correctly', () => {
            ...
            fc.assert(fc.property(stepArb, progressArb, (step, progress) => {
        ...
        test('should categorize user actions correctly', () => {
            const actionArb = fc.oneof(fc.constantFrom('grammar', 'style', 'clarity', 'tone', 'custom'), fc.constantFrom('opensettings', 'configure', 'apikey'), fc.constantFrom('help', 'about', 'documentation'), fc.string().filter(s => !['grammar', 'style', 'clarity', 'tone', 'custom',
        ...
        test('should handle responses with no changes gracefully', () => {
            ...
            fc.assert(fc.property(responseWithoutChangesArb, (response) => {
                const feedback = userFeedback.formatMultipleCorrectionsExplanation(response);
    ...
    describe('Unit Tests', () => {
        ...
        test('should generate clear labels for each correction type', () => {
            const grammarLabel = userFeedback.generateCorrectionTypeLabel(CorrectionType_1.CorrectionType.GRAMMAR);
            ...
            const styleLabel = userFeedback.generateCorrectionTypeLabel(CorrectionType_1.CorrectionType.STYLE);
...
//# sourceMappingURL=UserFeedback.test.js.map