...
describe('Prompt Configuration Tests', () => {
    ...
    describe('Property-Based Tests', () => {
        ...
        test('should persist and retrieve custom prompts correctly', () => {
            ...
            fc.assert(fc.property(validNameArb, validPromptArb, (name, promptContent) => {
                const customPrompt = {
                    id: '1',
                    name: name.trim(),
        ...
        test('should validate and reject invalid name-prompt pairs', () => {
            ...
            fc.assert(fc.property(invalidNamePromptArb, ({ name, prompt }) => {
        ...
        test('should accept valid name-prompt pairs', () => {
            ...
            const validPromptArb = fc.string({ minLength: 10, maxLength: 2000 })
                .filter(s => s.trim().length >= 10 && !/^\s*$/.test(s));
            fc.assert(fc.property(validNameArb, validPromptArb, (name, prompt) => {
        ...
        test('should create name-prompt pairs and make them available', async () => {
            ...
            const mockConfig = {
                ...
                update: jest.fn().mockImplementation(async (key, value) => {
                    if (key === 'customPrompts') {
                        storedPrompts = value;
                    }
                })
            ...
            require('vscode').workspace.getConfiguration = jest.fn().mockReturnValue(mockConfig);
            try {
                await fc.assert(fc.asyncProperty(validNameArb, validPromptArb, validDescriptionArb, async (name, prompt, description) => {
        ...
        test('should update existing name-prompt pairs', async () => {
            ...
            require('vscode').workspace.getConfiguration = jest.fn().mockReturnValue(mockConfig);
            try {
                await fc.assert(fc.asyncProperty(validNameArb, validPromptArb, validNameArb, validPromptArb, async (originalName, originalPrompt, newName, newPrompt) => {
        ...
        test('should delete name-prompt pairs and remove them from availability', async () => {
            ...
            require('vscode').workspace.getConfiguration = jest.fn().mockReturnValue(mockConfig);
            try {
                await fc.assert(fc.asyncProperty(validNameArb, validPromptArb, async (name, prompt) => {
                    // Reset and create initial pair
                    storedPrompts = [];
                    const namePromptPair = {
                        name: name.trim(),
