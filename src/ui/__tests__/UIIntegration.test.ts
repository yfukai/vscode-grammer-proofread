...
class ExtensionActivatorLogic {
    ...
    activate(): { success: boolean; commandsRegistered: string[] } {
...
class CommandRegistryLogic {
    private registeredCommands: Map<string, CorrectionType | 'settings' | 'custom'> = new Map();

    registerCommands(): { success: boolean; commandCount: number } {
        this.registeredCommands.set('grammarProofreading.correctGrammar', CorrectionType.GRAMMAR);
        this.registeredCommands.set('grammarProofreading.correctStyle', CorrectionType.STYLE);
        this.registeredCommands.set('grammarProofreading.correctClarity', CorrectionType.CLARITY);
        this.registeredCommands.set('grammarProofreading.correctTone', CorrectionType.TONE);
    ...
    getCommandMapping(commandId: string): CorrectionType | 'settings' | 'custom' | undefined {
    ...
    validateCommandId(commandId: string): { isValid: boolean; hasCorrectPrefix: boolean } {
...
class NotificationManagerLogic {
    formatSuccessMessage(response: CorrectionResponse): string {
    ...
    formatConfigurationError(errors: string[]): string {
        return `Configuration issues found:\n${errors.join('\n')}`;
    }

    generateDetailedHtml(response: CorrectionResponse): string {
    ...
    validateNotificationContent(content: string): { hasEmoji: boolean; hasMessage: boolean; length: number } {
...
describe('UI Integration', () => {
    let extensionActivator: ExtensionActivatorLogic;
    let commandRegistry: CommandRegistryLogic;
    let notificationManager: NotificationManagerLogic;
    ...
    describe('Command Registration', () => {
        test('should register all correction commands with proper mapping', () => {
            ...
            expect(commandRegistry.getCommandMapping('grammarProofreading.correctGrammar')).toBe(CorrectionType.GRAMMAR);
            expect(commandRegistry.getCommandMapping('grammarProofreading.correctStyle')).toBe(CorrectionType.STYLE);
            expect(commandRegistry.getCommandMapping('grammarProofreading.correctClarity')).toBe(CorrectionType.CLARITY);
            expect(commandRegistry.getCommandMapping('grammarProofreading.correctTone')).toBe(CorrectionType.TONE);
    ...
    describe('Notification Management', () => {
        test('should format success messages correctly', () => {
            const responseWithChanges: CorrectionResponse = {
        ...
        test('should format success message for single change', () => {
            const responseWithOneChange: CorrectionResponse = {
                correctedText: 'Corrected text',
                explanation: 'Fixed grammar',
                changes: [
                    { original: 'wrong', corrected: 'right', reason: 'grammar', position: { start: 0, end: 5 } }
